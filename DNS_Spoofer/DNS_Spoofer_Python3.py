...
def process_packet(packet):
    ...
    if scapy_packet.haslayer(scapy.DNSRR): #DNSRR is DNS Responce DNSRQ is DNS Request
        ...
        if "put destnation" in str(qname):
            ...
            packet.set_payload(bytes(scapy_packet)) #applying the changes we just made to the new packet that is going to be sent out
