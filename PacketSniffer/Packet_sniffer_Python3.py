...
#This is a python packet sniffer for python 3
...
def get_login_info(packet):
    if packet.haslayer(scapy.Raw):
        load = str(packet[scapy.Raw].load)  # printing the raw layer of the packet and the load feild of the packet
...
def process_sinffed_packet(packet): #pinting out the packet info that is only log in info
    if packet.haslayer(http.HTTPRequest):
        url = get_url(packet)
        print("[+] HTTP Request >> " + url.decode()) #.decode changes the data type of url to a str
